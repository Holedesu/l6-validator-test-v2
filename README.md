...
Этот валидатор с помощью метода `isValid()` проверяет входящие данные на соответствие тому, что они являются функцией.
Также в данный валидатор нужно добавить метод `callWith()`, который позволит вызывать функцию в переданном контексте.
И метод  `expect()`, который позволяет проверить, что наша функция вернула нам заданное значение.
...
schema2.isValid(()=>1); // false;
schema2.isValid(1); // false;

const schema3 = v.function().callWith({prop: 1}).expect('1'); 
schema3.isValid(() => '1'); // true;
schema3.isValid(() => 1); // false;
schema3.isValid(function () { return this.prop }); // true;
```
